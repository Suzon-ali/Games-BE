...
export const initRedisSubscribers = (io: Server) => {
  ...
  redisSubscriber.on('pmessage', (pattern, channel, message) => {
    try {
      const parsed = JSON.parse(message);
      if (pattern === 'wallet:update:*' && parsed?.userId) io.to(parsed.userId).emit('wallet:update', parsed);
      if (pattern === 'user:bet:placed:*' && parsed?.userId) io.to(parsed.userId).emit('user:bet:placed', parsed);
    } catch (err) {
